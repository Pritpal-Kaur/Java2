...
public class OrganizationDaoDB implements OrganizationDao {
    ...
    public List<Organization> getAllOrganizations() {
        ...
        for (Organization org : organizations) {
            org.setMembers(getHeroesForOrganization(org));
        }
    ...
    private List<Hero> getHeroesForOrganization(Organization organization) {
        final String GET_SUPERS = "SELECT h.id, h.name, h.description, h.superpowerId "
                + "FROM OrganizationMember om "
                + "JOIN hero h ON om.heroId = h.id "
                + "WHERE om.organizationId = ?";
        List<Hero> heroes = jdbc.query(GET_SUPERS, new HeroMapper(), organization.getId());
